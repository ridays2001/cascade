...
const useScroll = (
	...
	useEffect(() => {
		if (listContainer.current && scrollableContainer.current && focusedIndex >= 0) {
			...
			const focusedChild = children && children.length ? (children[focusedIndex] as HTMLDivElement) : null;

			if (focusedChild && focusedChild.getBoundingClientRect) {
				const { height: optionHeight } = focusedChild.getBoundingClientRect();
				const { height: listHeight } = scrollableContainer.current.getBoundingClientRect();
...
type Action = { type: 'setFocusIndex'; focusIndex: number } | { type: 'toggleFocus'; isFocus: boolean };
